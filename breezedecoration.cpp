...
K_PLUGIN_FACTORY_WITH_JSON(
   // BreezeDecoFactory,
   BreezeSierraDecoFactory, "breeze.json",
...
QColor Decoration::titleBarColor() const {
    ...
    if (hideTitleBar())
        ...
        return KColorUtils::mix(
           ...
           c->color(ColorGroup::Active, ColorRole::TitleBar), m_opacity);
    } else
        return c->color(
           c->isActive() ? ColorGroup::Active : ColorGroup::Inactive,
           ColorRole::TitleBar);
}
...
QColor Decoration::fontColor() const {
    ...
    if (m_animation->state() == QPropertyAnimation::Running) {
        return KColorUtils::mix(
           ...
           c->color(ColorGroup::Active, ColorRole::Foreground), m_opacity);
    } else {
        return c->color(
           c->isActive() ? ColorGroup::Active : ColorGroup::Inactive,
...
void Decoration::init() {
    ...
    connect(s.data(), &KDecoration2::DecorationSettings::reconfigured,
            SettingsProvider::self(), &SettingsProvider::reconfigure,
            Qt::UniqueConnection);
    connect(s.data(), &KDecoration2::DecorationSettings::reconfigured, this,
            &Decoration::updateButtonsGeometryDelayed);
    ...
    connect(c, &KDecoration2::DecoratedClient::adjacentScreenEdgesChanged, this,
            &Decoration::recalculateBorders);
    connect(c, &KDecoration2::DecoratedClient::maximizedHorizontallyChanged,
            this, &Decoration::recalculateBorders);
    connect(c, &KDecoration2::DecoratedClient::maximizedVerticallyChanged, this,
            &Decoration::recalculateBorders);
    connect(c, &KDecoration2::DecoratedClient::shadedChanged, this,
            &Decoration::recalculateBorders);
...
void Decoration::updateTitleBar() {
    ...
    const int width =
       maximized
          ? c->width()
          : c->width() - 2 * s->largeSpacing() * Metrics::TitleBar_SideMargin;
    const int height =
       maximized
          ? borderTop()
          : borderTop() - s->smallSpacing() * Metrics::TitleBar_TopMargin;
    const int x =
       maximized ? 0 : s->largeSpacing() * Metrics::TitleBar_SideMargin;
    const int y =
       maximized ? 0 : s->smallSpacing() * Metrics::TitleBar_TopMargin;
...
void Decoration::updateAnimationState() {
    if (m_internalSettings->animationsEnabled()) {
        auto c = client().data();
        m_animation->setDirection(c->isActive() ? QPropertyAnimation::Forward
...
void Decoration::updateSizeGripVisibility() {
    auto c = client().data();
    if (m_sizeGrip) {
        m_sizeGrip->setVisible(c->isResizeable() && !isMaximized()
                               && !c->isShaded());
    }
}
...
void Decoration::updateButtonsGeometry() {
    ...
    foreach (const QPointer<KDecoration2::DecorationButton> &button,
        ...
        button.data()->setGeometry(
           QRectF(QPoint(0, 0), QSizeF(bWidth, bHeight)));
        static_cast<Button *>(button.data())
           ->setOffset(QPointF(0, verticalOffset));
        static_cast<Button *>(button.data())
           ->setIconSize(QSize(bWidth, bWidth));
    }
    ...
    if (!m_leftButtons->buttons().isEmpty()) {
        ...
        if (isLeftEdge()) {
            // add offsets on the side buttons, to preserve padding, but satisfy
            // Fitts law
            auto button =
               static_cast<Button *>(m_leftButtons->buttons().front().data());
            button->setGeometry(
               QRectF(QPoint(0, 0), QSizeF(bWidth + hPadding, bHeight)));
    ...
    if (!m_rightButtons->buttons().isEmpty()) {
        ...
        if (isRightEdge()) {
            auto button =
               static_cast<Button *>(m_rightButtons->buttons().back().data());
            button->setGeometry(
               QRectF(QPoint(0, 0), QSizeF(bWidth + hPadding, bHeight)));
            ...
            m_rightButtons->setPos(
               QPointF(size().width() - m_rightButtons->geometry().width()
    ...
    if (!c->isShaded()) {
        ...
        if (!hideTitleBar())
            painter->setClipRect(0, borderTop(), size().width(),
                                 size().height() - borderTop(),
                                 Qt::IntersectClip);

        if (s->isAlphaChannelSupported())
            painter->drawRoundedRect(rect(), Metrics::Frame_FrameRadius,
...
void Decoration::paintTitleBar(QPainter *painter, const QRect &repaintRegion) {
    ...
    // TODO Review this. Here the window color is appended in
    // matchedTitleBarColor var
    ...
    if (c->isActive() && m_internalSettings->drawBackgroundGradient()) {
        ...
        const QColor titleBarColor =
           (matchColorForTitleBar() ? matchedTitleBarColor
                                    : this->titleBarColor());
        ...
        // TODO Review this. Initialize titleBarColor based on user's choise.
        // I needed another else if because the window might not be active or
        // has drawBackgroundGradient but I still need to take care the konsole
        // case.
        const QColor titleBarColor =
           (matchColorForTitleBar() ? matchedTitleBarColor
                                    : this->titleBarColor());
    ...
    if (isMaximized() || !s->isAlphaChannelSupported()) {
        ...
        painter->drawRoundedRect(titleRect, Metrics::Frame_FrameRadius,
                                 Metrics::Frame_FrameRadius);

    } else {
        painter->setClipRect(titleRect, Qt::IntersectClip);

        // the rect is made a little bit larger to be able to clip away the
        // rounded corners at the bottom and sides
        painter->drawRoundedRect(
           titleRect.adjusted(isLeftEdge() ? -Metrics::Frame_FrameRadius : 0,
        ...
        switch (m_internalSettings->titleAlignment()) {
            ...
            case InternalSettings::AlignCenterFullWidth: {
                ...
                const QRect fullRect =
                   QRect(0, yOffset, size().width(), captionHeight());
                QRect boundingRect(settings()
                                      ->fontMetrics()
                                      .boundingRect(c->caption())
                                      .toRect());

                // text bounding rect
                boundingRect.setTop(yOffset);
                boundingRect.setHeight(captionHeight());
                boundingRect.moveLeft((size().width() - boundingRect.width())
                                      / 2);
...
void Decoration::createShadow() {
    ...
    if (!g_sShadow || g_shadowSize != m_internalSettings->shadowSize()
        ...
        const int shadowOffset =
           qMax(6 * g_shadowSize / 16, Metrics::Shadow_Overlap * 2);

        // create image
        QImage image(2 * g_shadowSize, 2 * g_shadowSize,
        ...
        QRadialGradient radialGradient(g_shadowSize, g_shadowSize,
                                       g_shadowSize);
        for (int i = 0; i < 10; ++i) {
            const qreal x(qreal(i) / 9);
            radialGradient.setColorAt(
               x,
               gradientStopColor(g_shadowColor, alpha(x) * g_shadowStrength));
        }
        ...
        // contrast pixel
        QRectF innerRect =
           QRectF(g_shadowSize - Metrics::Shadow_Overlap,
                  g_shadowSize - shadowOffset - Metrics::Shadow_Overlap,
                  2 * Metrics::Shadow_Overlap,
                  shadowOffset + 2 * Metrics::Shadow_Overlap);
        // g_shadowSize - shadowOffset - Metrics::Shadow_Overlap, g_shadowSize -
        // shadowOffset - Metrics::Shadow_Overlap, shadowOffset +
        // 2*Metrics::Shadow_Overlap, shadowOffset + 2*Metrics::Shadow_Overlap
        // );

        painter.setPen(
           gradientStopColor(g_shadowColor, g_shadowStrength * 0.5));
        painter.setBrush(Qt::NoBrush);
        painter.drawRoundedRect(innerRect, -0.5 + Metrics::Frame_FrameRadius,
                                -0.5 + Metrics::Frame_FrameRadius);
...
void Decoration::createSizeGrip(void) {
    ...
    if (c->windowId() != 0) {
        ...
        connect(c, &KDecoration2::DecoratedClient::maximizedChanged, this,
                &Decoration::updateSizeGripVisibility);
        connect(c, &KDecoration2::DecoratedClient::shadedChanged, this,
                &Decoration::updateSizeGripVisibility);
        connect(c, &KDecoration2::DecoratedClient::resizeableChanged, this,
                &Decoration::updateSizeGripVisibility);
    }
#endif
}
