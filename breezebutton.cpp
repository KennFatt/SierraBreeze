...
Button::Button(DecorationButtonType type, Decoration *decoration,
    ...
    // connections
    connect(decoration->client().data(), SIGNAL(iconChanged(QIcon)), this,
            SLOT(update()));
    connect(decoration->settings().data(),
            &KDecoration2::DecorationSettings::reconfigured, this,
            &Button::reconfigure);
    connect(this, &KDecoration2::DecorationButton::hoveredChanged, this,
...
Button::Button(QObject *parent, const QVariantList &args)
    // : DecorationButton(args.at(0).value<DecorationButtonType>(),
    // args.at(1).value<Decoration*>(), parent) , m_flag(FlagStandalone) ,
    // m_animation( new QPropertyAnimation( this ) )
    // {}
    : Button(args.at(0).value<DecorationButtonType>(),
             args.at(1).value<Decoration *>(), parent) {
    m_flag = FlagStandalone;
    //! icon size must return to !valid because it was altered from the default
    //! constructor, in Standalone mode the button is not using the decoration
    //! metrics but its geometry
    ...
    if (auto d = qobject_cast<Decoration *>(decoration)) {
        ...
        switch (type) {
            case DecorationButtonType::Close:
                ...
                QObject::connect(
                   d->client().data(),
                   &KDecoration2::DecoratedClient::closeableChanged, b,
            ...
            case DecorationButtonType::Maximize:
                ...
                QObject::connect(
                   d->client().data(),
                   &KDecoration2::DecoratedClient::maximizeableChanged, b,
                   &SierraBreeze::Button::setVisible);
                break;
            ...
            case DecorationButtonType::Minimize:
                ...
                QObject::connect(
                   d->client().data(),
                   &KDecoration2::DecoratedClient::minimizeableChanged, b,
                   &SierraBreeze::Button::setVisible);
                break;
            ...
            case DecorationButtonType::ContextHelp:
                ...
                QObject::connect(
                   ...
                   b, &SierraBreeze::Button::setVisible);
                break;
            ...
            case DecorationButtonType::Shade:
                ...
                QObject::connect(
                   d->client().data(),
                   &KDecoration2::DecoratedClient::shadeableChanged, b,
                   &SierraBreeze::Button::setVisible);
                break;

            case DecorationButtonType::Menu:
                QObject::connect(d->client().data(),
                                 &KDecoration2::DecoratedClient::iconChanged, b,
                                 [b]() { b->update(); });
                break;
        ...
        switch (type()) {
            ...
            case DecorationButtonType::KeepBelow: {
                ...
                if (isChecked() || isHovered()) {
                    painter->setPen(hint_pen);
                    painter->drawPolyline(QPolygonF()
                                          << QPointF(4, 5) << QPointF(9, 10)
                                          << QPointF(14, 5));

                    painter->drawPolyline(QPolygonF()
                                          << QPointF(4, 9) << QPointF(9, 14)
                                          << QPointF(14, 9));
                }
            ...
            case DecorationButtonType::ContextHelp: {
                ...
                path.cubicTo(QPointF(12.5, 9.5), QPointF(9, 7.5),
                             QPointF(9, 11.5));
        ...
        if (type() == DecorationButtonType::Close) {
            if (d->internalSettings()->outlineCloseButton()) {
                return KColorUtils::mix(
                   d->fontColor(),
                   c->color(ColorGroup::Warning, ColorRole::Foreground)
                      .lighter(),
                   m_opacity);
...
void Button::updateAnimationState(bool hovered) {
    ...
    m_animation->setDirection(hovered ? QPropertyAnimation::Forward
