...
SizeGrip::SizeGrip(Decoration* decoration)
    ...
    // mask
    QPolygon p;
    p << QPoint(0, GripSize) << QPoint(GripSize, 0)
      << QPoint(GripSize, GripSize) << QPoint(0, GripSize);
    ...
    connect(c, &KDecoration2::DecoratedClient::heightChanged, this,
            &SizeGrip::updatePosition);
    connect(c, &KDecoration2::DecoratedClient::activeChanged, this,
            &SizeGrip::updateActiveState);
...
void SizeGrip::updateActiveState(void) {
    ...
    if (QX11Info::isPlatformX11()) {
        ...
        xcb_configure_window(QX11Info::connection(), winId(),
    ...
    if (windowId) {
        ...
        */
        ...
        xcb_query_tree_cookie_t cookie =
           xcb_query_tree_unchecked(connection, current);
        ScopedPointer<xcb_query_tree_reply_t> tree(
...
void SizeGrip::updatePosition(void) {
    ...
    auto c = m_decoration.data()->client().data();
    QPoint position(c->width() - GripSize - Offset,
...
void SizeGrip::sendMoveResizeEvent(QPoint position) {
    ...
    */
    ...
    ScopedPointer<xcb_get_geometry_reply_t> reply(
       xcb_get_geometry_reply(connection, cookie, 0x0));
    if (reply) {
        // translate coordinates
        xcb_translate_coordinates_cookie_t coordCookie(
           xcb_translate_coordinates(connection, winId(), reply.data()->root,
    ...
    if (!m_moveResizeAtom) {
        ...
        ScopedPointer<xcb_intern_atom_reply_t> reply(
           xcb_intern_atom_reply(connection, cookie, 0x0));
